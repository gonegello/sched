...
$config = new Config();
$records = new Reports();
$source_income = new Source_Income();
$result = $source_income->getWhere(" AND status = 'A'");
...
if(!empty($source)){
    $resRecords = $records->getMonthLy15(" AND soi.name = '$source'");
     #age 0-6
     $mm_0_6 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 0 AND 6 )");
     $male_0_6 = count($mm_0_6);

     $ff_0_6 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 0 AND 6 )");
     $female_0_6 = count($ff_0_6);

     #age 7-12
     $mm_7_12 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 7 AND 12 )");
     $male_7_12 = count($mm_7_12);

     $ff_7_12 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 7 AND 12 )");
     $female_7_12 = count($ff_7_12);

     #age 13-19
     $mm_13_19 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 13 AND 19 )");
     $male_13_19 = count($mm_13_19);

     $ff_13_19 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 13 AND 19 )");
     $female_13_19 = count($ff_13_19);

     #age 20-30
     $mm_20_30 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 20 AND 30 )");
     $male_20_30 = count($mm_20_30);

     $ff_20_30 = $records->getMonthLy15("  AND soi.name = '$source' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 20 AND 30 )");
     $female_20_30 = count($ff_20_30);

     #age 20-30
     $mm_31_59 = $records->getMonthLy15("  AND soi.name = '$source' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 31 AND 59 )");
     $male_31_59 = count($mm_31_59);

     $ff_31_59 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 31 AND 59 )");
     $female_31_59 = count($ff_31_59);

     #age above 60
     $mm_60 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 60 AND 100 )");
     $male_60 = count($mm_60);

     $ff_60 = $records->getMonthLy15(" AND soi.name = '$source' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
     + 0 BETWEEN 60 AND 100 )");
     $female_60 = count($ff_60);
...
<head>
    ...
    <title>Major Source of Income By Age and Sex | Print</title>
...
<body>
    ...
    <tr>
            <td colspan="8" style="text-align: center;border:none;"><h3>
                MAJOR SOURCE OF INCOME OF THE POPULATION (15 years old and above) BY AGE AND SEX
            </h3></td>
        </tr>
    ...
    <table>
        ...
        foreach($resRecords AS $row): $i++;?>
            ...
            <td style="border:1px solid black;text-align:center;"><?php echo $row['major_source'];?></td>
    ...
    <table style="width:90%">
        ...
        <td valign="top">
            ...
            <td valign="top">
                <h5>SUMMARY OF MAJOR SOURCE OF INCOME 15 YRS AND ABOVE</h5>
                <table class="households-type" style="width: 100%;">
                    <thead class="thead-households-type">  
                        <tr>
                    ...
                    foreach($result AS $row):
                        $name = $row['name'];
                        $re = $records->getMonthLy15(" AND soi.name = '$name' AND (DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
