...
$schooltypes = ['PU', 'PR'];
$schooltypelist = [];
foreach($schooltypes as $key => $school) {
    $count = $educationalinfo->getschooltypereport(" AND school_type = '$school'");
    $schooltypelist[] = [
        'type' => ($school == 'PU') ? 'Public' : 'Private',
        'count' => $count
    ];
}
...
if(!empty($grade)){
    if(!empty($sex)) #if not empty type - not empty sex - not empty age
    {
        ...
        $resRecords = $records->getJoinSchool(" AND gender = '$sex' AND sch.school_type = '$grade'
        AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') + 0 BETWEEN 0 AND 6 )");

            if($sex == 'M')
            {
                $mm = $records->getJoinSchool(" AND gender = 'M' AND sch.school_type = '$grade'
                AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') + 0 BETWEEN 0 AND 6 )");
                $male = count($mm);
            }
            if($sex == 'F')
            {
                $ff = $records->getJoinSchool(" AND gender = 'F' AND sch.school_type = '$grade'
                AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') + 0 BETWEEN 0 AND 6 )");
                $female = count($ff);
            }
        }
        ...
        if($age == '13-19')
        {
            ...
            if($sex == 'M')
            {
                $mm = $records->getJoinSchool(" AND gender = 'M' AND sch.school_typel = '$grade'
                AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') + 0 BETWEEN 13 AND 19 )");
                $male = count($mm);
            }
            if($sex == 'F')
    ...
    if(empty($sex)) # empty sex - not empty age
    {
        ...
        $resRecords = $records->getJoinSchool(" AND sch.school_type = '$grade'
        AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') + 0 BETWEEN 0 AND 6 )");
        $count = count($resRecords);
        ...
        if(empty($age)) #if not empty type - not empty sex - but empty age
        {
            $resRecords = $records->getJoinSchool(" AND sch.school_type = '$grade'");
             #age 0-6
             $mm_0_6 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 0 AND 6 )");
             $male_0_6 = count($mm_0_6);
 
             $ff_0_6 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 0 AND 6 )");
             $female_0_6 = count($ff_0_6);
 
             #age 7-12
             $mm_7_12 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 7 AND 12 )");
             $male_7_12 = count($mm_7_12);
 
             $ff_7_12 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 7 AND 12 )");
             $female_7_12 = count($ff_7_12);
 
             #age 13-19
             $mm_13_19 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 13 AND 19 )");
             $male_13_19 = count($mm_13_19);
 
             $ff_13_19 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 13 AND 19 )");
             $female_13_19 = count($ff_13_19);
 
             #age 20-30
             $mm_20_30 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 20 AND 30 )");
             $male_20_30 = count($mm_20_30);
 
             $ff_20_30 = $records->getJoinSchool("  AND sch.school_type = '$grade' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 20 AND 30 )");
             $female_20_30 = count($ff_20_30);
 
             #age 20-30
             $mm_31_59 = $records->getJoinSchool("  AND sch.school_type = '$grade' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 31 AND 59 )");
             $male_31_59 = count($mm_31_59);
 
             $ff_31_59 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 31 AND 59 )");
             $female_31_59 = count($ff_31_59);
 
             #age above 60
             $mm_60 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'M' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 60 AND 100 )");
             $male_60 = count($mm_60);
 
             $ff_60 = $records->getJoinSchool(" AND sch.school_type = '$grade' AND p.gender = 'F' AND (  DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),p.birthdate)), '%Y') 
             + 0 BETWEEN 60 AND 100 )");
...
//Get brgy captain
$brgycaptain = $config->getSettings("AND name = 'barangaycaptain'");

$brgycaptainname = $brgycaptain['value'];
...
<body>
    <table>
        <tr>
            <td colspan="7" style="text-align: center;border:none;"><h3>CURRENTLY ENROLLED REPORT BY AGE AND SEX (SCHOOL TYPE)</h3></td>
        </tr>
            ...
            <td colspan="7" style="text-align: center;border:none;text-transform:uppercase;"><h4>
                ...
                if(!empty($grade))
                {
                    if(!empty($sex))
                    {
                        if(!empty($age))
                        {
                            echo '( '; if($grade == 'PU'){ echo 'Public';}else{ echo 'Private';}; echo ' : '; echo $newSex ; echo ' : '; echo $age; echo ' )';
                        }
                        if(empty($age))
                        {
                            echo '( ';if($grade == 'PU'){ echo 'Public';}else{ echo 'Private';}; echo ' : '; echo $newSex ; echo ' : '; echo 'All Age )';
                    ...
                    if(empty($sex))
                    {
                        if(!empty($age))
                        {
                            echo '( '; if($grade == 'PU'){ echo 'Public';}else{ echo 'Private';}; echo ' : '; echo 'All Sex' ; echo ' : '; echo $age; echo ' )';
                        }
                        if(empty($age))
                        {
                           echo '( '; if($grade == 'PU'){ echo 'Public';}else{ echo 'Private';}; echo ' : '; echo 'All Sex' ; echo ' : '; echo 'All Age )'; 
                ...
                if(empty($grade))
                {
                    if(!empty($sex))
                    {
                        if(!empty($age))
                        {
                            echo '( All School Type'; echo ' : '; echo $newSex ; echo ' : '; echo $age; echo ' )';
                        }
                        if(empty($age))
                        {
                            echo '( All School Type'; echo ' : '; echo $newSex ; echo ' : '; echo 'All Age )';
                    ...
                    if(empty($sex))
                    {
                        if(!empty($age))
                        {
                            echo '( All School Type'; echo ' : '; echo 'All Sex' ; echo ' : '; echo $age; echo ' )';
                        }
                        if(empty($age))
                        {
                            echo '( All Records )';
        ...
        <tr>
            ...
            <td style="text-align: center;border:1px solid black;font-weight:bold;" rowspan="2">SCHOOL TYPE</td>
    ...
    <br>
    <br>
    <?php echo $helpers->getsignage($brgysecretary, 'all', $brgycaptainname) ?>
</body>
</html>