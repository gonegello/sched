...
$columns = ['firstname','lastname', 'value', 'created_at'];

$whereCondition = $sqlTot = $sqlRec = '';
$whereCondition = " AND department_id = $id ";
if( !empty($params['search']['value']) ) {
    $whereCondition .= " AND ";
    $whereCondition .= " ( firstname LIKE '%". $params['search']['value'] ."%' )";
...
//Get total
$totalRecords = $instructors->getTotal($whereCondition, $sortBy);

//Get all tsag
$results = $instructors->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($results AS $row) {
    ...
    data-department_id="'. $row['department_id'] .'" data-firstname="'. $row['firstname'] .'" 
    data-middle_initial="'. $row['middle_initial'] .'" data-lastname="'. $row['lastname'] .'"
    data-status="'. $row['status'] .'"><i class="bi bi-pencil-square"></i></a>';
    $action .= '&nbsp; <a class="btn btn-circle btn-sm btn-danger btn-delete" data-id="'. $row['id'] .'" data-firstname="'. $row['firstname'] .'"><i class="bi bi-x"></i></a>';
...
$baseUrl = $protocol . $_SERVER['HTTP_HOST'] . '/views/instructors/'.$btnAction.'.php?' . $urlFormer;
