<?php
require ('../../inc/Helpers.php');
require ('../../inc/app_settings.php');
require ('../../models/Logs.php'); 

if(!isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    echo json_encode(['code' => 4, 'message' => 'You are not authorized to access this page']);
    return;
}

$logs = new Logs();

//Helpers
$helpers = new Helpers();

$btnAction  = isset($_POST['action']) ? $_POST['action'] : '';

$params = $columns = $totalRecords = $data = [];
 
$params = $_REQUEST;
$urlFormer = '';

if(in_array($btnAction, ['excel', 'print'])) {
    $_SESSION['SESS_GEN_TOKEN'] = rand(10000, 10000000);

    $urlFormer = 'token='. $_SESSION['SESS_GEN_TOKEN'];
}

$columns = ['u.first_name', 'u.last_name', 'l.page','l.action'];

$whereCondition = $sqlTot = $sqlRec = '';

if( !empty($params['search']['value']) ) {

    if(stripos('unknown', $params['search']['value']) !== false) {
        $whereCondition .= " AND l.userid = 0";
    } else {
        $whereCondition .= " AND ";
        $whereCondition .= " ( first_name LIKE '%". $params['search']['value'] ."%' ";
        $whereCondition .= " OR u.last_name LIKE '%". $params['search']['value'] ."%' )";
    }
    $urlFormer .= '&search_value=' . $params['search']['value'];
}

$sortBy = 'l.id DESC';

if(isset($params['order'])) {
    $sortBy = $columns[$params['order'][0]['column']]."   ". $params['order'][0]['dir'];
    $urlFormer .= '&sort_by=' . $columns[$params['order'][0]['column']];
    $urlFormer .= '&sort_type=' . $params['order'][0]['dir'];
}

$start  = $params['start'];
$length = $params['length'];

//Get total
$totalRecords = $logs->getTotal($whereCondition, $sortBy);

//Get all tsag
$results = $logs->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($results AS $row) {
    $encryptedId = $helpers->encryptDecrypt($row['id']);
    
    $action = '<a class="btn btn-circle btn-sm btn-primary btn-view" data-id="'. $row['id'] .'"><i class="bi bi-eye-fill"></i></a>';
    $data[] = [
        $row['first_name'],
        $row['last_name'],
        $row['page'],
        $row['action'],
        $row['log'],
        date('M d, Y h:i a', strtotime($row['created_at'])),
        $action,
    ];
}

$baseUrl = BASE_URL . '/views/audit-log/'.$btnAction.'.php?' . $urlFormer;

$json_data = [
    "draw"            => intval( $params['draw'] ),   
    "recordsTotal"    => intval( $totalRecords ),  
    "recordsFiltered" => intval($totalRecords),
    "data"            => $data,
    'url'             => $baseUrl
];

echo json_encode($json_data);