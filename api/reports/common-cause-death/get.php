...
require ('../../../models/Death_information.php'); 
...
$death_information = new Death_informations();
...
$columns = ['name', 'duplicates'];

$whereCondition = $sqlTot = $sqlRec = '';

if( !empty($params['search']['value']) ) {
    $whereCondition .= " AND ( d.cause_of_deaths LIKE '%". $params['search']['value'] ."%' )";
    $whereCondition .= " AND ( d.personal_info_id LIKE '%". $params['search']['value'] ."%' )";
...
//Get total
$totalRecords = $death_information->getTotal($whereCondition, $sortBy);

//Get all tsag
$results = $death_information->getcausejoin($whereCondition, $sortBy, $start, $length);
$data = [];
